      ...
      >
        ...
        <div className="flex p-4 pb-0 gap-2">
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${mode === 'bulk' ? 'bg-primary-600 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
          >
            Em Lote
          </button>
          <button
            onClick={() => setMode('single')}
            className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${mode === 'single' ? 'bg-primary-600 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
          >
            Único
          </button>
        </div>
            ...
            <>
              ...
              {preview.length > 0 && (
                <div className="bg-primary-50 rounded-lg p-3 border border-primary-100">
                  ...
                  <p className="text-xs text-primary-600">{preview.join(', ')}</p>
                </div>
              )}
            </>
          )}
